    ...
    try:
        ...
        if response.data:
            ...
            existing_data = supabase.table('companies') \
                                    .select('company_id', 'co2_emission') \
            ...
            for company_id, avg_co2 in avg_emissions.items():
                ...
                print(avg_co2)
                print(old_co2)
                co2_difference = round(avg_co2 - old_co2, 2)  # Differenza tra nuova e vecchia emissione
...
def update_co2_emission_for_seller():
    try:
        ...
        if response.data:
            ...
            # Recupera i valori attuali delle emissioni dalla tabella "companies"
...
def run_scheduler():
    # Registra le compagnie al primo avvio
    register_companies_with_eth()
    ...
    # Aggiunge gli schedule per il blockchain
    schedule.every(10).minutes.do(register_companies_with_eth)  # Controlla nuove compagnie ogni 10 minuti
    schedule.every(10).seconds.do(update_blockchain_emissions)  # Aggiorna le emissioni ogni 10 minuti
